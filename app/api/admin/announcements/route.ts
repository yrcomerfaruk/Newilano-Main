...
export async function GET() {
  ...
  const docs = await AnnouncementModel.find().sort({ order: 1, createdAt: 1 }).lean();
  const announcements = docs.map((d: any) => ({ id: d._id.toString(), message: String(d.message), active: !!d.active, order: Number(d.order) || 0 }));
